...
static void _Task_Emerg_Stop( void *pvParameters ){
	...
	while(1){
		...
		//https://support.xilinx.com/s/question/0D52E00006hpPAoSAM/turn-on-an-led-with-a-button?language=en_US
		...
		if(pressedCount >= 3){
			...
			//currentposition_in_step = targetPosition_InSteps-decelerationDistance_InSteps;
			Stepper_getCurrentPositionInSteps(targetPosition_InSteps-decelerationDistance_InSteps);


			for(int i=0; i<sequenceIndex; i++){
				positionSequence[i][0]=0;
				positionSequence[i][1]=0;
			}
			...
			{
				//https://support.xilinx.com/s/question/0D52E00006hpPAoSAM/turn-on-an-led-with-a-button?language=en_US
			    //XGpioPs_WritePin(&Red_RGBInst, RGB_LED_DEVICE_ID, 1);
				XGpio_DiscreteWrite(&Red_RGBInst,1,RGB_LED);
			    vTaskDelay(200);
			    //XGpioPs_WritePin(&Red_RGBInst, RGB_LED_DEVICE_ID, 0);
