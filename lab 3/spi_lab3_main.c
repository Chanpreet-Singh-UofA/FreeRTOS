	...
	while(1){
		...
		if(flag==1){ 						// flag is set to 1 when the user enters the end sequence on SPI0-SPI1 mode
			...
			for(int i=0;i<loop_counter;i++){
				...
				if (XUartPs_IsTransmitFull(XPAR_XUARTPS_0_BASEADDR) == FALSE){
...
static void TaskSpi0Master( void *pvParameters ){
	...
	while(1){
		...
		if(spi_master_loopback_en==1 && current_command_execution_flag==2) 				//just send the characters back to task 1. No SPI access! This is because loopback is currently enabled.
			...
			send_buffer[1] = task2_receive_from_FIFO1; 
...
static void TaskSpi1Slave( void *pvParameters ){
	...
	while(1){
		...
		if(spi_master_loopback_en==0 && current_command_execution_flag==2){
			...
			//Keep sending the bytes back to SPI master until termination sequence
			if (end_sequence_flag != 3){
				temp_store = *RxBuffer_Slave;        //Making temp_store as our Sendbuffer to master
				SpiSlaveRead(TRANSFER_SIZE_IN_BYTES);
				num_received ++;
				

				//Implementation of checkForTerminationSequence() here 
			...
			//Termination sequence detected therefore we send the message string 
			else{
				xil_printf("The number of characters received over SPI:%d\n", num_received);

				char console_message[80] = "The number of characters received over SPI:%d\n";
				str_length = sprintf(buffer,console_message,num_received);
				//Looping mechanism to send the message string to master
				for (int i =0; i < str_length; i++){
					SpiSlaveWrite(&buffer[i], TRANSFER_SIZE_IN_BYTES);
				}
                
				//Reset the given values
				end_sequence_flag = 0; 
				num_received = 0; 
