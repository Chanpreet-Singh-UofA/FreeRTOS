...
u32 loop_count = 24000;  //GLOBAL variable for loop_count to generate a fake CPU load
u32 run;
...
static void TaskPrintRunTimeStats( void *pvParameters )
{
	...
	while(1){
		...
		*/
		...
		xil_printf(pcWriteBuffer);

		//xil_printf( "\nPrinting run time stats final (loop count = %d)", loop_count);
		run ++;
		vTaskDelay(delay);
	}
}
